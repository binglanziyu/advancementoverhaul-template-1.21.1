 ...
 *   <tr><td>{@link ConditionType 本类}</td><td>枚举</td><td>9 种条件类型</td></tr>
 *   <tr><td>{@link NbtMatchMode 本类}</td><td>枚举</td><td>4 种 NBT 匹配模式</td></tr>
 *   <tr><td>{@link DataSource 本类}</td><td>枚举</td><td>条件数据源类别</td></tr>
 *   <tr><td>{@link com.example.advancementoverhaul.data.model.CustomAdvancement model/}</td><td>类</td><td>自定义进度</td></tr>
 *   <tr><td>{@link com.example.advancementoverhaul.data.model.AdvancementCondition model/}</td><td>类</td><td>进度条件</td></tr>
 *   <tr><td>{@link com.example.advancementoverhaul.data.model.VanillaAdvMeta model/}</td><td>类</td><td>原版进度元数据</td></tr>
 * </table>
 */
public class DataStore {
    ...
    // 中文标识（历史兼容，不建议在新代码中使用）
    // TODO: 在下一个数据格式版本迁移中切换到英文标识
    ...
    public static String getTabDisplayName(String tab) {
        return switch (tab) {
            case TAB_DEFAULT -> net.minecraft.network.chat.Component.translatable(
                    com.example.advancementoverhaul.LangKeys.TAB_DEFAULT).getString();
            case TAB_VANILLA -> net.minecraft.network.chat.Component.translatable(
                    com.example.advancementoverhaul.LangKeys.TAB_VANILLA_DISPLAY).getString();
            default -> tab;
        };
    }
