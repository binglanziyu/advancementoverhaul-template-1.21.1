...
public final class ResourceLoader {
    private static final Logger LOGGER = LoggerFactory.getLogger((String)"AdvancementOverhaul/Resources");
    private static final ResourceLocation PLAQUE_TEX = ResourceLocation.fromNamespaceAndPath((String)"advancementoverhaul", (String)"custom_plaque");
    private static boolean initialized = false;
    private static boolean plaqueAvailable = false;
    private static Path chimePath;
    private static boolean chimeAvailable;
    ...
    public static void init(Path configDir) {
        ...
        ResourceLoader.ensureDir(resDir);
        ResourceLoader.generateReadme(resDir);
        Path plaquePath = resDir.resolve("plaque.png");
        if (Files.exists(plaquePath, new LinkOption[0])) {
            LOGGER.info("Found custom plaque texture: {}", (Object)plaquePath);
        }
        if (Files.exists(mp3Path = resDir.resolve("chime.mp3"), new LinkOption[0])) {
            if (ResourceLoader.validateMp3Header(mp3Path)) {
                chimePath = mp3Path;
                chimeAvailable = true;
                LOGGER.info("Found custom chime (MP3): {}", (Object)mp3Path);
            } else {
        ...
        for (String ext : List.of(".wav", ".ogg", ".flac", ".aac", ".m4a", ".mp4", ".avi", ".mkv", ".mov", ".webm")) {
            ...
            if (!Files.exists(wrong, new LinkOption[0])) continue;
            LOGGER.warn("Unsupported chime format '{}' ignored \u2014 only .mp3 is supported", (Object)wrong.getFileName());
        }
    }
        ...
        try {
            Thread thread = new Thread(() -> {
                try (AudioInputStream audioIn = AudioSystem.getAudioInputStream(chimePath.toFile());){
                    ...
                    LOGGER.warn("Failed to play custom chime: {}", (Object)e.getMessage());
                }
            }, "AO-Chime");
            ...
            LOGGER.warn("Failed to start custom chime playback: {}", (Object)e.getMessage());
            return false;
        }
    }
    ...
    private static void ensureDir(Path dir) {
        try {
            ...
            LOGGER.warn("Failed to create resource directory: {}", (Object)e.getMessage());
        }
    }

    private static void generateReadme(Path resDir) {
        ResourceLoader.generateReadmeCn(resDir);
        ResourceLoader.generateReadmeEn(resDir);
    }
    ...
    private static void generateReadmeCn(Path resDir) {
        ...
        try {
            Files.writeString(readme, (CharSequence)content, new OpenOption[0]);
            LOGGER.info("Generated resource README at {}", (Object)readme);
        }
        catch (IOException e) {
            LOGGER.warn("Failed to generate README: {}", (Object)e.getMessage());
        }
    }
    ...
    private static void generateReadmeEn(Path resDir) {
        ...
        try {
            Files.writeString(readme, (CharSequence)content, new OpenOption[0]);
            LOGGER.info("Generated resource README_EN at {}", (Object)readme);
        }
        catch (IOException e) {
            LOGGER.warn("Failed to generate README_EN: {}", (Object)e.getMessage());
        }
    }

    static {
        chimeAvailable = false;
    }
}
