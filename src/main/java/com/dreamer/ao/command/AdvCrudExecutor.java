...
final class AdvCrudExecutor {
    ...
    /**
     * 批量删除进度。需要附带 "confirm" 关键词避免误操作。
     * 格式：{@code /adv batchdelete <id1,id2,...> confirm}
     * <p>
     * 不带 confirm 时仅预览，不执行删除。
     */
    static int batchDelete(CommandContext<CommandSourceStack> ctx) {
        if (!CommandHelper.checkPerm(ctx)) return 0;
        String idsStr = StringArgumentType.getString(ctx, "ids");
        if (!CommandHelper.validateGreedyInput(ctx, "ids", idsStr)) return 0;
        String trimmed = idsStr.trim();

        // Require explicit "confirm" to prevent accidental bulk deletion
        if (!trimmed.endsWith(" confirm")) {
            List<String> previewIds = new ArrayList<>();
            for (String id : trimmed.split(",")) {
                id = id.trim();
                if (!id.isEmpty()) previewIds.add(id);
            }
            ctx.getSource().sendFailure(
                    Component.translatable(LangKeys.CMD_BATCH_DELETE_CONFIRM,
                            String.join(", ", previewIds), previewIds.size()));
            return 0;
        }

        // Strip " confirm" suffix and parse
        String confirmed = trimmed.substring(0, trimmed.lastIndexOf(" confirm")).trim();

        ServerDataStore store = ServerDataStore.getInstance();
        int count = 0;
        List<String> toDelete = new ArrayList<>();
        List<String> notFound = new ArrayList<>();
        for (String id : confirmed.split(",")) {
            id = id.trim();
    ...
    static int setName(CommandContext<CommandSourceStack> ctx) {
        ...
        if (!CommandHelper.validateGreedyInput(ctx, "data", data)) return 0;
    ...
    static int createFromJson(CommandContext<CommandSourceStack> ctx) {
        ...
        if (!CommandHelper.validateGreedyInput(ctx, "json", json)) return 0;
        try {
    ...
    static int updateFromJson(CommandContext<CommandSourceStack> ctx) {
        ...
        if (!CommandHelper.validateGreedyInput(ctx, "json", raw)) return 0;
        try {
            // Base64 解码（兼容旧格式）
            String json;
            try {
