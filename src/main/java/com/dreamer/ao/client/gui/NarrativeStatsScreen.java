...
extends NarrativeScreen {
    private static final Logger LOGGER = LoggerFactory.getLogger(NarrativeStatsScreen.class);
    ...
    private static String formatCustomStat(PlayerStats stats, String fieldName) {
        ...
        if (fieldName.equals("furthestDistance")) {
            double distance = stats.getFurthestDistance();
            if (distance == 0.0) return "\u2014";
            return String.format("%.1f m", distance);
        }
        if (fieldName.equals("mostFrequentBiome")) {
            if (stats.getMostFrequentBiome() == null || stats.getMostFrequentBiome().isEmpty()) return "\u2014";
            return formatBiomeName(stats.getMostFrequentBiome());
        }
        if (val == 0L) return "\u2014";
    ...
    private static NarrativeResult formatNarrativeField(PlayerStats stats, String actualKey) {
        return switch (actualKey) {
            case "firstNetherDay"     -> dayResult(stats.getFirstNetherDay());
            case "firstEndDay"        -> dayResult(stats.getFirstEndDay());
            case "firstDiamondDay"    -> dayResult(stats.getFirstDiamondDay());
            case "firstEnchantDay"    -> dayResult(stats.getFirstEnchantDay());
            case "firstTameDay"       -> dayResult(stats.getFirstTameDay());
            case "firstRainSleepDay"  -> dayResult(stats.getFirstRainSleepDay());
            case "firstDeathDay"      -> stats.isFirstDeathRecorded()
                    ? new NarrativeResult(Component.translatable("advancementoverhaul.narrative.day", stats.getFirstDeathDay()).getString(),
                                          formatCoords(stats.getFirstDeathX(), stats.getFirstDeathY(), stats.getFirstDeathZ()))
                    : new NarrativeResult("\u2014", null);
            case "latestDeath"        -> stats.isFirstDeathRecorded()
                    ? new NarrativeResult("", formatCoords(stats.getLatestDeathX(), stats.getLatestDeathY(), stats.getLatestDeathZ()))
                    : new NarrativeResult("\u2014", null);
            case "firstBlockPlaced"   -> stats.isFirstBlockPlacedRecorded()
                    ? new NarrativeResult("", formatCoords(stats.getFirstBlockPlacedX(), stats.getFirstBlockPlacedY(), stats.getFirstBlockPlacedZ()))
                    : new NarrativeResult("\u2014", null);
            case "lowestY"            -> stats.hasLowestY() ? new NarrativeResult("Y=" + stats.getLowestY(), null) : new NarrativeResult("\u2014", null);
            case "highestY"           -> stats.hasHighestY() ? new NarrativeResult("Y=" + stats.getHighestY(), null) : new NarrativeResult("\u2014", null);
