        ...
        if (panel.edIcon != null && !panel.edIcon.isEmpty()) {
            ...
            if (panel.edIcon.startsWith("entity:")) {
                ...
                if (rl != null) {
                    ...
                    if (item != null) {
                        if (panel.screen.showSel) {
                            // 选择器弹窗激活时跳过3D渲染，避免图标穿透弹窗
                            g.drawString(font, GuiUtils.truncate(font, new ItemStack(item).getHoverName().getString(), w - 8), x + 4, y + 5, TEXT, false);
                        } else {
